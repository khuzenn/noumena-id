...
class Catatan_Sipil extends CI_Controller {
	 ...
	 */
     ...
     public function __construct()
     {
		...
		$this->load->model('Catatan_sipil_model');
     }

	public function detail ($id_artikel) {
		$this->data['url'] = site_url('catatan-sipil');

		$detail_artikel = $this->Catatan_sipil_model->getDetailArtikel(decrypt_url($id_artikel));
        ...
        $this->load->view('catatan-sipil/detail', $this->data);
	...
	public function load_more()
    {
        ...
        $catatan = $this->Catatan_sipil_model->getMoreArtikelCatatanSipil($limit, $offset);

        $this->data['list_artikel'] = $catatan;

		foreach ($this->data['list_artikel'] as &$article) {
			$article->isi = $this->limitWords($article->isi, 50); 
		}

        $this->load->view('catatan-sipil/load_more', $this->data);
    }
     ...
     public function index()
	{
		...
		$this->data['list_artikel'] = $this->Catatan_sipil_model->getArtikelCatatanSipil($limit);
		...
		$this->load->view('template/header');
        $this->load->view('template/nav');
        $this->load->view('catatan-sipil/views',$this->data);
        $this->load->view('template/footer');
	}
}
