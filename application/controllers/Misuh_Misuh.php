...
class Misuh_Misuh extends CI_Controller {
	 ...
	 */
     ...
     public function __construct()
     {
		...
		$this->load->model('Misuh_misuh_model');
     }

	public function detail ($id_artikel) { 
		$this->data['url'] = site_url('misuh-misuh');

		$detail_artikel = $this->Misuh_misuh_model->getDetailArtikel(decrypt_url($id_artikel));
        ...
        $this->load->view('misuh-misuh/detail', $this->data);
	...
	public function load_more()
    {
        ...
        $misuh = $this->Misuh_misuh_model->getMoreArtikelMisuhMisuh($limit, $offset);

        $this->data['list_artikel'] = $misuh;

		foreach ($this->data['list_artikel'] as &$article) {
			$article->isi = $this->limitWords($article->isi, 50); 
		}

        $this->load->view('misuh-misuh/load_more', $this->data);
    }

    public function index()
	{
		$limit = $this->session->userdata('limit') ? $this->session->userdata('limit') : 3;
		$this->data['list_artikel'] = $this->Misuh_misuh_model->getArtikelMisuhMisuh($limit);
		...
		$this->load->view('template/header');
        $this->load->view('template/nav');
        $this->load->view('misuh-misuh/views', $this->data);
        $this->load->view('template/footer');
	}
}
